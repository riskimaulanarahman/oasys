			...
			if($auth){
				switch ($this->post['criteria']){
					...
					case 'update':
						...
						$join   = "LEFT JOIN tbl_approver ON (tbl_iteieapproval.approver_id = tbl_approver.id) ";
						if (isset($data['mode'])){
							$Iteieapproval = Iteieapproval::find('first', array('joins'=>$join,'conditions' => array("iteie_id=? and tbl_approver.employee_id=? and ApprovalStatus = 0",$doid,$Employee->id),'include' => array('approver'=>array('employee','approvaltype'))));
							unset($data['mode']);
						}else{
