...
Class Advpaymentmodule extends Application{
				...
				switch ($this->post['criteria']){
					...
					case 'update':
						...
						if (isset($data['superior'])){
							...
							$dx = Advpaymentapproval::find('all',array('joins'=>$joins,'conditions' => array("advpayment_id=? and tbl_approver.approvaltype_id=35 and not(tbl_approver.employee_id=?)",$id,$superior)));	
							...
							$Advpaymentapproval = Advpaymentapproval::find('all',array('joins'=>$joins,'conditions' => array("advpayment_id=? and tbl_approver.employee_id=?",$id,$superior)));	
	...
	function generatePDFi($id){
		...
		$superiorId=$Advpayment->superior;
		...
		try {
			...
			// }
				...
				foreach ($Advpaymentapproval as $data){
					if(($data->approver->approvaltype->id==35) || ($data->approver->employee_id==$Advpayment->superior)){
