			...
			if($auth){
				switch ($this->post['criteria']){
					...
					case 'update':
						...
						$join   = "LEFT JOIN tbl_approver ON (tbl_itsharefapproval.approver_id = tbl_approver.id) ";
						if (isset($data['mode'])){
							$Itsharefapproval = Itsharefapproval::find('first', array('joins'=>$join,'conditions' => array("itsharef_id=? and tbl_approver.employee_id=? and (ApprovalStatus=0 or ApprovalStatus=4)",$doid,$Employee->id),'include' => array('approver'=>array('employee','approvaltype'))));
							unset($data['mode']);
						}else{
