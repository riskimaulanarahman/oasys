						...
						if(isset($query['status'])){
							switch ($query['status']){
								case 'last':
									...
									$companyFC=(($company=="BCL") ||  ($company=='KPA')  )?"KPSI":((($company=='KPS') || ($company=='KPSI')) ?"LDU":$company);
					...
					case 'create':		
						...
						try{
							...
							// }else{
								...
								$companyFC=(($data['companycode']=='BCL') || ($data['companycode']=='KPA'))?"KPSI":((($data['companycode']=='KPS') || ($data['companycode']=='KPSI'))?"LDU":$Employee->companycode);
