...
Class DayoffModule extends Application{
	...
	function generatePDF(){
		...
		$pdfContent .= '<tr><td>*Personnel Number (SAP ID)</td><td>:</td><td style="border:solid windowtext 1.0pt;padding:2.5pt 5.4pt 2.5pt 5.4pt;">'.$emp['sapid'].'</td><td style="width=10px;"></td><td>*Superior Name</td><td>:</td><td style="border:solid windowtext 1.0pt;padding:2.5pt 5.4pt 2.5pt 5.4pt;">'.$supname.'</td></tr>
						...
						<tr><td>Business Group </td><td>:</td><td style="border:solid windowtext 1.0pt;padding:2.5pt 5.4pt 2.5pt 5.4pt;">'.$comp['companycode'].'</td><td colspan=4></td></tr>
					...
					case 'find':
						...
						if(isset($query['status'])){
							...
							if($Employee->location->sapcode=='0200'){
								...
								$Dayoff = Dayoff::find('all',array('joins'=>$joinx,'conditions' => array('tbl_dayoffreq.RequestStatus=3 and tbl_employee.company_id=?',$Employee->company_id),'include' => array('employee'=>array('company','department'))));
							}
						...
						try {
							...
							if (count($dd)>0){
								...
								if((date('N',strtotime($data['dateworked']))==7) && (count($do)>0)){
									...
									$max = ($sun+$hol)-2;
									$joins   = "LEFT JOIN tbl_dayoffreq as r ON (dayoff_id = r.id) ";	
									$do = Dayoffdetail::find('all', array('joins'=>$joins,'conditions' => array("month(dateworked)=? and r.employee_id=? and not(tbl_dayoffdetail.id=?)",date("m", strtotime($data['dateworked'])),$Employee->id,$id),'include'=>array("dayoff")));
									if (count($do)>=$max){
