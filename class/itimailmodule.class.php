							...
							switch ($query['status']){
										...
										if (($formtype=='1')){
												...
												if(count($Itimailapproval)==0){
													...
													if((substr(strtolower($Employee->location->sapcode),0,3)=="020") || ($Employee->department->sapcode=="13000090") || ($Employee->department->sapcode=="13000121") || ($Employee->company->sapcode=="NKF") || ($Employee->company->sapcode=="RND")){
														...
														$ApproverCADKF = Approver::find('first',array('joins'=>$joinx,'conditions'=>array("module='IT' and tbl_approver.isactive='1' and approvaltype_id='30' and tbl_employee.location_id='8'")));
											...
											if(count($Itimailapprovalmd)==0){
												if($Employee->companycode == 'BCL') {
													$ApproverMD = Approver::find('first',array('joins'=>$joinx,'conditions'=>array("module='IT' and tbl_approver.isactive='1' and approvaltype_id='33' and tbl_employee.companycode='BCL' ")));
												} else {
													$ApproverMD = Approver::find('first',array('joins'=>$joinx,'conditions'=>array("module='IT' and tbl_approver.isactive='1' and approvaltype_id='33' and not(tbl_employee.companycode='BCL') ")));
												}
												if(count($ApproverMD)>0){
														$Itimailapproval = new Itimailapproval();
														$Itimailapproval->itimail_id = $id;
														$Itimailapproval->approver_id = $ApproverMD->id;
														$Itimailapproval->save();
														$logger = new Datalogger("Itimailapproval","add","Add Approval MD",json_encode($Itimailapproval->to_array()));
														$logger->SaveData();
												}
								...
								case 'appreq':
									...
									if(isset($query['accessreq'])) {
										...
										if ($accessreq == 2){
											$Appl = Itimailapproval::find('all',array('joins'=>$joins,'conditions' => array("itimail_id=? and tbl_approver.approvaltype_id=30 and tbl_employee.location_id ='8' ",$id)));
					...
					case 'create':
						...
						try{
							...
							$joinx   = "LEFT JOIN tbl_employee ON (tbl_approver.employee_id = tbl_employee.id) ";
								...
								// 	// $Approver2 = Approver::find('first',array('joins'=>$joinx,'conditions'=>array("module='IT' and tbl_approver.isactive='1' and approvaltype_id=30 and tbl_employee.location_id='8'")));
								// 	// if(count($Approver2)>0){
								// 	// 	$Itimailapproval = new Itimailapproval();
								// 	// 	$Itimailapproval->itimail_id = $Itimail->id;
								// 	// 	$Itimailapproval->approver_id = $Approver2->id;
