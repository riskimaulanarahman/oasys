(function (app) {
app.register.controller('advanceCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
    $scope.ds={};
    $scope.test=[];
	$scope.disabled= true;
    ...
    var myStore = new DevExpress.data.CustomStore({
		load: function() {			
            $scope.isLoaded =true;
            return CrudService.GetAll('advancebyemp').then(function (response) {
		...
		byKey: function(key) {
            CrudService.GetById('advance',encodeURIComponent(key)).then(function (response) {
				return response;
			});
		},
		...
		update: function(key, values) {
            CrudService.Update('advance',key.id,values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
                    ...
                    cellTemplate: function (container, options) {
                        $('<div style="padding:2px 15px 2px 15px;"/>').addClass('dx-icon-detailslayout btn-pill btn-shadow btn btn-primary')
                            ...
                            .on('dxclick', function () {
								...
								$scope.loadAdvance(options.data,"view",true);
                            })
                            .appendTo(container);
						if((options.data.requeststatus=='0') || (options.data.requeststatus=='2')){	
                            ...
                            .on('dxclick', function () {
								...
								// } else{
									$scope.loadAdvance(options.data,"edit",true);
								// }
                            })
                            .appendTo(container);
						} else if((options.data.approveddoc=='' || options.data.approveddoc==null)  && (options.data.requeststatus=='3')){
			...
			{						
				...
				options: {
					...
					onClick: function() {
						...
						$scope.loadAdvance({createddate:d,username:$rootScope.curUser.username},"add",true);
					}
				}
			});
        },
