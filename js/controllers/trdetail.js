(function (app) {
	app.register.controller('trdetailCtrl', ['$rootScope', '$scope', '$http', '$interval', '$location', 'CrudService', 'AuthenticationService', '$filter', function ($rootScope, $scope, $http, $interval, $location, CrudService, AuthenticationService, $filter) {
		...
		$scope.travelcategoryOpt = [
			'Internal BU', 'External BU'
		]
					...
					items: [{
						...
						items: [
							...
							},
							{
								...
								editorOptions: {
									readOnly: (($scope.mode == 'edit') || ($scope.mode == 'add')) ? false : true,
									dataSource: $scope.travelcategoryOpt,
									layout: "horizontal",
									onValueChanged: function (e) {
										$scope.formInstance.updateData('travelcategory', e.value);
									}
								},
								validationRules: [{
									type: "required",
									message: "Please choose Travel Category"
								}]
							},
							{
		...
		$scope.tabs = [
			...
			{ id: 5, TabName: "SupportDoc", title: 'Supporting Document', template: "tab4" },
		...
		$scope.gridOptions = {
			...
			columns: [
				{ dataField: 'departdate', validationRules: [{ type: "required" }], width: 100, caption: "Depart Date", dataType: "date", format: 'dd/MM/yyyy', editorType: "dxDateBox", 
					editorOptions: { 
						displayFormat: "dd/MM/yyyy", 
						min: ($scope.mode == 'reschedule') ? new Date(d.getFullYear(), d.getMonth(), d.getDate() - 1000) : new Date(d.getFullYear(), d.getMonth(), d.getDate() - 1), 
				...
				{ dataField: 'arrivingdate', validationRules: [{ type: "required" }], width: 100, caption: "Arriving Date", dataType: "date", format: 'dd/MM/yyyy', editorType: "dxDateBox", 
					editorOptions: { 
						displayFormat: "dd/MM/yyyy", 
						// min: new Date(d.getFullYear(), d.getMonth(), d.getDate() - 1), 
						min: ($scope.mode == 'reschedule') ? new Date(d.getFullYear(), d.getMonth(), d.getDate() - 1000) : new Date(d.getFullYear(), d.getMonth(), d.getDate() - 1), 
						disabled: (($scope.mode == 'approve') || ($scope.mode == 'view') || ($scope.mode == 'report')) ? true : false 
					} 
				},
		...
		var myStore4 = new DevExpress.data.CustomStore({
			load: function () {
				$scope.isLoaded = true;
				return CrudService.GetById('trfile', $scope.Requestid);
			},
			byKey: function (key) {
				CrudService.GetById('trfile', encodeURIComponent(key)).then(function (response) {
					return response;
				});
			},
			insert: function (values) {
				...
				CrudService.Create('trfile', values).then(function (response) {
					if (response.status == "error") {
						DevExpress.ui.notify(response.message, "error");
					}
					$scope.grid2Component.refresh();
			...
			update: function (key, values) {
				...
				CrudService.Update('trfile', key.id, values).then(function (response) {
					if (response.status == "error") {
						DevExpress.ui.notify(response.message, "error");
					}
					$scope.grid2Component.refresh();
			...
			remove: function (key) {
				CrudService.Delete('trfile', key.id).then(function (response) {
					if (response.status == "error") {
						DevExpress.ui.notify(response.message, "error");
					}
					$scope.grid2Component.refresh();
		...
		$scope.grid5Options = {
			...
			onEditorPreparing: function (e) {
				...
				if (e.dataField == "FileLoc") {
					...
					e.editorOptions.uploadUrl = "api.php?action=uploadtrfile&id=" + $scope.Requestid;
