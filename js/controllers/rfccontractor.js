...
app.register.controller('rfccontractorCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
    ...
    var myStore = new DevExpress.data.CustomStore({
		...
		byKey: function(key) {
            CrudService.GetById('rfccontractor',encodeURIComponent(key)).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				return response;
		...
		insert: function(values) {
            CrudService.Create('rfccontractor',values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
		...
		update: function(key, values) {
            CrudService.Update('rfccontractor',key.id,values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
		...
		remove: function(key) {
			CrudService.Delete('rfccontractor',key.id).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
    ...
    $scope.dataGridOptions = {
        ...
        columns: [{caption: '#',fixed: true,formItem: { visible: false},width: 40,
				  ...
				  ,{dataField:'contractorcode',caption:"Contractor Code"}
				  ,{dataField:'contractorname',caption:"Contractor Name"}
