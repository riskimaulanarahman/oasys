(function (app) {
app.register.controller('spklCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
    $scope.ds={};
    $scope.test=[];
	$scope.disabled= true;
    ...
    var myStore = new DevExpress.data.CustomStore({
		load: function() {			
            $scope.isLoaded =true;
            return CrudService.GetAll('spklbyemp').then(function (response) {
		...
		byKey: function(key) {
            CrudService.GetById('spkl',encodeURIComponent(key)).then(function (response) {
				return response;
			});
		},
		insert: function(values) {
            CrudService.Create('spkl',values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
		...
		update: function(key, values) {
            CrudService.Update('spkl',key.id,values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
		...
		remove: function(key) {
			CrudService.Delete('spkl',key.id).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
			...
			{						
				...
				options: {
					...
					onClick: function() {
						...
						$scope.loadSPKL({createddate:d,username:$rootScope.curUser.username},"add",true);
					}
				}
			});
        },
