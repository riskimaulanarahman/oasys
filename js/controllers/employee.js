    ...
    $scope.dataGridOptions = {
        ...
        columns: [{
                    ...
                    cellTemplate: function (container, options) {
                        $('<div style="padding:2px 15px 2px 15px;"/>').addClass('dx-icon-detailslayout dx-button dx-button-success')
                            .text('')
                            .on('dxclick', function () {
                                DevExpress.ui.notify("Loading detail data for "+options.data.designationname,"info",600);
								$scope.loadEmployee(options.data,"view");
                            })
                            .appendTo(container);
						$('<div style="padding:2px 15px 2px 15px;"/>').addClass('dx-icon-edit dx-button dx-button-success')
                            .text('')
                            .on('dxclick', function () {
								if (!$scope.allowEdit){
									DevExpress.ui.notify("You don't have authority to edit data","error");
								} else{
									$scope.loadEmployee(options.data,"edit");
								}							
                            })
                            .appendTo(container);
                    }
                  ...
                  ,{caption: '#',fixed: true,formItem: { visible: false},width: 40,
				   ...
				   {dataField:'sapid',caption:'SAP ID'},
