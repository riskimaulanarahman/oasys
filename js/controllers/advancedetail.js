(function (app) {
	app.register.controller('AdvancedetailCtrl', ['$rootScope', '$scope', '$http', '$interval', '$location', 'CrudService', 'AuthenticationService', '$filter', function ($rootScope, $scope, $http, $interval, $location, CrudService, AuthenticationService, $filter) {
		...
		CrudService.GetById('advance', $scope.Requestid).then(function (response) {
			if (response.status == "autherror") {
				...
				$scope.detailFormOptions = {
					...
					},
					{
						...
						colCount: 2,
						items: [
							// waktu pemberian yang di harapkan
							{
			...
			$scope.grid1Options = {
				...
				columns: [
					{
						dataField: 'description', width: 150, dataType: "string", validationRules: [{ type: "required" }], editorOptions: {
							...
							disabled: (($scope.mode == 'approve') || ($scope.mode == 'view'))
								? true
								: false
						}
					},
					...
					},
					{
						...
						editorOptions: {
							format: "fixedPoint",
							disabled: (($scope.mode == 'approve') || ($scope.mode == 'view'))
								? (($rootScope.isAdmin) || (($scope.data.apprstatuscode == 2 && $scope.mode == 'approve'))
									? false
									: true)
								: false
						}
					...
					{
						dataField: 'remarks', width: 150, dataType: "string",
						editorOptions: {
							disabled: (($scope.mode == 'approve') || ($scope.mode == 'view'))
								? (($rootScope.isAdmin) || (($scope.data.apprstatuscode == 2 && $scope.mode == 'approve'))
									? false
				...
				onRowInserted: function (e) {
					...
					criteria = { status: 'appcon', formtype: formadv, advance_id: $scope.Requestid, employee_id: $scope.data.employee_id, isbudgeted: $scope.data.isbudgeted };
					CrudService.FindData('advance', criteria).then(function (response) {
						$scope.grid2Component.refresh();
					})
				},
				onRowUpdated: function (e) {
					// console.log($scope.data.isbudgeted);
					// var amount = e.component.getTotalSummaryValue("amount");
					var formadv = $('#advformtype').val();
					// console.log(amount);
					console.log(formadv);
		...
		$scope.updateAdvance = function (e) {
			...
			else if ($scope.data.isbudgeted == null && ($scope.data.apprstatuscode == 5)) {
				DevExpress.ui.notify({
					message: "Please select budget category",
					type: "warning",
					displayTime: 5000,
					height: 80,
					position: {
