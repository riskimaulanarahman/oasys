(function (app) {
    app.register.controller('mmf30Ctrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
        $scope.ds={};
        $scope.test=[];
        $scope.disabled= true;
        ...
        var myStore = new DevExpress.data.CustomStore({
            load: function() {			
                $scope.isLoaded =true;
                return CrudService.GetAll('mmf30byemp').then(function (response) {
            ...
            insert: function(values) {
                CrudService.Create('mmf30',values).then(function (response) {
                    if(response.status=="error"){
                         DevExpress.ui.notify(response.message,"error");
                    }
            ...
            update: function(key, values) {
                CrudService.Update('mmf30',key.id,values).then(function (response) {
                    if(response.status=="error"){
                         DevExpress.ui.notify(response.message,"error");
                    }
                    $scope.dataGrid.refresh();
            ...
            remove: function(key) {
                CrudService.Delete('mmf30',key.id).then(function (response) {
                    if(response.status=="error"){
                         DevExpress.ui.notify(response.message,"error");
                    }
                    $scope.dataGrid.refresh();
                        ...
                        cellTemplate: function (container, options) {
                            $('<div style="padding:2px 15px 2px 15px;" title="View Detail" />').addClass('dx-icon-detailslayout btn-pill btn-shadow btn btn-primary')
                                ...
                                .on('dxclick', function () {
                                    ...
                                    $scope.loadMMF30(options.data,"view",true);
                                })
                                .appendTo(container);
                ...
                {						
                    ...
                    options: {
                        ...
                        onClick: function() {
                            ...
                            $scope.loadMMF30({createddate:d,username:$rootScope.curUser.username},"add",true);
                        }
                    }
                });
            },
