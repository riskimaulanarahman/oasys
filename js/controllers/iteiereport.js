(function (app) {
    app.register.controller('iteiereportCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
        $scope.ds={};
        $scope.test=[];
        $scope.disabled= true;
        ...
        var myStore = new DevExpress.data.CustomStore({
            load: function() {			
                ...
                //end filter date
                return CrudService.FindData('iteieapp',criteria).then(function (response) {
            ...
            insert: function(values) {
                // CrudService.Create('iteie',values).then(function (response) {
                //     if(response.status=="error"){
                //          DevExpress.ui.notify(response.message,"error");
                //     }
                //     $scope.dataGrid.refresh();
            ...
            update: function(key, values) {
                ...
                CrudService.Update('iteie',key.id,values).then(function (response) {
                    if(response.status=="error"){
                         DevExpress.ui.notify(response.message,"error");
                    }
                    $scope.dataGrid.refresh();
            ...
            remove: function(key) {
                // CrudService.Delete('iteie',key.id).then(function (response) {
                //     if(response.status=="error"){
                //          DevExpress.ui.notify(response.message,"error");
                //     }
                //     $scope.dataGrid.refresh();
        ...
        //start filter date
        $scope.$on("initITEIE", function(event, name) {
            initController();
        });
                        ...
                        cellTemplate: function (container, options) {
                            $('<div style="padding:2px 15px 2px 15px;" title="View Detail" />').addClass('dx-icon-detailslayout btn-pill btn-shadow btn btn-primary')
                                ...
                                .on('dxclick', function () {
                                    ...
                                    $scope.loadITEIE(options.data,"report",true);
                                })
                                .appendTo(container);
                                if((options.data.approveddoc=='' || options.data.approveddoc==null)  && (options.data.requeststatus=='3')){
                                    $('<div style="padding:2px 15px 2px 15px;"/>').addClass('dx-icon-repeat  btn-pill btn-shadow btn btn-success')
                                        .text('')
                                        .on('dxclick', function () {
                                            // window.open('api/apiiteiepdf?id='+options.data.id, '_blank');
                                            DevExpress.ui.notify("Please Wait...","info");
                                            $.ajax({
                                                url: 'api/apiiteiepdf?id='+options.data.id,
                                                type: 'get',
                                                cache: false,
