(function (app) {
    app.register.controller('iteieCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
        $scope.ds={};
        $scope.test=[];
        $scope.disabled= true;
        ...
        var myStore = new DevExpress.data.CustomStore({
            load: function() {			
                $scope.isLoaded =true;
                return CrudService.GetAll('iteiebyemp').then(function (response) {
            ...
            byKey: function(key) {
                CrudService.GetById('iteie',encodeURIComponent(key)).then(function (response) {
                    return response;
                });
            },
                        ...
                        cellTemplate: function (container, options) {
                            $('<div style="padding:2px 15px 2px 15px;" title="View Detail" />').addClass('dx-icon-detailslayout btn-pill btn-shadow btn btn-primary')
                                ...
                                .on('dxclick', function () {
                                    ...
                                    $scope.loadITEIE(options.data,"view",true);
                                })
                                .appendTo(container);
                            if((options.data.requeststatus=='0') || (options.data.requeststatus=='2')){
                                ...
                                .on('dxclick', function () {
                                    ...
                                    // } else{
                                        $scope.loadITEIE(options.data,"edit",true);
                                    // }
                                })
                                .appendTo(container);
                            }else{
