(function (app) {
app.register.controller('moduleCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService)  {
    $scope.ds={};
    $scope.test=[];
	$scope.disabled= true;
    ...
    var myStore = new DevExpress.data.CustomStore({
		load: function() {			
            $scope.isLoaded =true;
            return CrudService.GetAll('module').then(function (response) {
		...
		byKey: function(key) {
            CrudService.GetById('module',encodeURIComponent(key)).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				return response;
		...
		insert: function(values) {
            CrudService.Create('module',values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
		...
		update: function(key, values) {
            CrudService.Update('module',key.id,values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
		...
		remove: function(key) {
			CrudService.Delete('module',key.id).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
    ...
    $scope.dataGridOptions = {
        ...
        columns: [{
                  ...
                  ,{dataField:'module',caption:"Module Name",fixed: true,fixedPosition: "left"}
