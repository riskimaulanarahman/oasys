(function (app) {
	app.register.controller('rfcCtrl', [
		'$rootScope',
		'$scope',
		'$http',
		'$interval',
		...
		function (
			...
			var myStore = new DevExpress.data.CustomStore({
				...
				byKey: function (key) {
					CrudService.GetById('rfc', encodeURIComponent(key)).then(
						function (response) {
							return response;
						},
					);
				...
				columns: [
					{
						...
						cellTemplate: function (container, options) {
							$('<div style="padding:2px 15px 2px 15px;"/>')
								...
								.appendTo(container);
							if (
								options.data.requeststatus == '0' ||
								options.data.requeststatus == '2'
							) {
								$('<div style="padding:2px 15px 2px 15px;"/>')
									.addClass('dx-icon-edit btn-pill btn-shadow btn btn-success')
									.text('')
									.on('dxclick', function () {
										// if (!$scope.allowEdit){
            ...
            },*/
						{
							...
							options: {
								...
								onClick: function () {
									...
									$scope.loadRFC(
										{ CreatedDate: d, username: $rootScope.curUser.username },
										'add',
										true,
									);
								},
