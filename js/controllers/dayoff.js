(function (app) {
app.register.controller('dayoffCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
    $scope.ds={};
    $scope.test=[];
	$scope.disabled= true;
    ...
    var myStore = new DevExpress.data.CustomStore({
		load: function() {			
            $scope.isLoaded =true;
            return CrudService.GetAll('dayoffbyemp').then(function (response) {
		...
		insert: function(values) {
            CrudService.Create('dayoff',values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
		...
		update: function(key, values) {
            CrudService.Update('dayoff',key.id,values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
		...
		remove: function(key) {
			CrudService.Delete('dayoff',key.id).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
                    ...
                    cellTemplate: function (container, options) {
                        $('<div style="padding:2px 15px 2px 15px;"/>').addClass('dx-icon-detailslayout btn-pill btn-shadow btn btn-primary')
                            ...
                            .on('dxclick', function () {
								...
								$scope.loadDayoff(options.data,"view",true);
                            })
                            .appendTo(container);
						if((options.data.requeststatus=='0') || (options.data.requeststatus=='2')){	
                            ...
                            .on('dxclick', function () {
								...
								// } else{
									$scope.loadDayoff(options.data,"edit",true);
								// }
                            })
                            .appendTo(container);
						}else{
			...
			{						
				...
				options: {
					...
					onClick: function() {
						...
						$scope.loadDayoff({RequestDate:d,username:$rootScope.curUser.username},"add",true);
					}
				}
			});
        },
