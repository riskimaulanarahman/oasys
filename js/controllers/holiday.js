...
app.register.controller('holidayCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
    ...
    var myStore = new DevExpress.data.CustomStore({
		load: function() {			
            ...
            return CrudService.GetAll('holiday').then(function (response) {
		...
		byKey: function(key) {
            CrudService.GetById('holiday',encodeURIComponent(key)).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				return response;
		...
		insert: function(values) {
			values.holidaydate = $filter("date")(values.holidaydate, "yyyy-MM-dd")
            CrudService.Create('holiday',values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
		...
		update: function(key, values) {
			values.holidaydate = $filter("date")(values.holidaydate, "yyyy-MM-dd")
            CrudService.Update('holiday',key.id,values).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
		...
		remove: function(key) {
			CrudService.Delete('holiday',key.id).then(function (response) {
				if(response.status=="error"){
					 DevExpress.ui.notify(response.message,"error");
				}
				$scope.dataGrid.refresh();
    ...
    $scope.dataGridOptions = {
        ...
        columns: [
                  ...
                  ,{dataField:'holidaydate',datatype:'date',caption:"Date",format: 'yyyy-MM-dd',editorType: "dxDateBox",fixed: true,fixedPosition: "left",editorOptions: {displayFormat:"yyyy-MM-dd"}}
				  ,{dataField:'description',caption:"Description",fixed: true,fixedPosition: "left"}
				  ,{dataField:'remarks',caption:"Remarks",fixed: true,fixedPosition: "left"}
                  ],
        "export": {
            enabled: true,
            fileName: "ExportGrid",
