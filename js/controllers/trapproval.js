(function (app) {
app.register.controller('trapprovalCtrl', ['$rootScope','$scope', '$http', '$interval','$location','CrudService','AuthenticationService','$filter', function($rootScope,$scope, $http, $interval,$location,CrudService,AuthenticationService,$filter)  {
    $scope.ds={};
    $scope.test=[];
	$scope.disabled= true;
	var myStore = new DevExpress.data.CustomStore({
		load: function() {			
            ...
            return CrudService.FindData('trapp',criteria).then(function (response) {
                    ...
                    cellTemplate: function (container, options) {
                        ...
                        $('<div style="padding:2px 15px 2px 15px;"/>').addClass(icon+'  btn-pill btn-shadow btn btn-primary')
                            ...
                            .on('dxclick', function () {
								...
								var  mode='approve';
								$scope.loadTR(options.data,mode,$scope.Filter);
                            })
                            .appendTo(container);
                    }
                }
