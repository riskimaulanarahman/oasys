(function (app) {
  app.register.controller("advpaymentreportCtrl", [
    "$rootScope",
    "$scope",
    "$http",
    "$interval",
    ...
    function (
      ...
      var myStore = new DevExpress.data.CustomStore({
        load: function () {
          ...
          //end filter date
          return CrudService.FindData("advpaymentapp", criteria).then(function (
            response
          ) {
        ...
        byKey: function (key) {
          CrudService.GetById("advpaymentapp", encodeURIComponent(key)).then(
            function (response) {
              return response;
            }
          );
        ...
        update: function (key, values) {
          ...
          CrudService.Update("advpayment", key.id, values).then(function (
            response
          ) {
            if (response.status == "error") {
              DevExpress.ui.notify(response.message, "error");
      ...
      $scope.dataGridOptions = {
        ...
        columns: [
            ...
            cellTemplate: function (container, options) {
              $(
                ...
                .on("dxclick", function () {
                  ...
                  $scope.loadAdvpayment(options.data, "report", true);
                })
                .appendTo(container);
              if (
                ...
                $('<div style="padding:2px 15px 2px 15px;"/>')
                  ...
                  .on("dxclick", function () {
                    ...
                    $.ajax({
                      url: "api/apiadvpaymentpdf?id=" + options.data.id,
                      type: "get",
                      cache: false,
          ...
          customizeText: function (e) {
            var rDesc = ["","<span class='mb-2 mr-2 badge badge-pill badge-info'>Payment Req HR</span>","<span class='mb-2 mr-2 badge badge-pill badge-danger'>Payment Req OPS</span>","<span class='mb-2 mr-2 badge badge-pill badge-primary'>HTI-HE</span>",""];
